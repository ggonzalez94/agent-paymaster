...
export class ServoRpcClient {
  ...
  async sendUserOperation(userOperation: UserOperation, entryPoint: string): Promise<`0x${string}`> {
  ...
  async getPaymasterData(
    ...
    return this.rpc<PaymasterRpcResult>("pm_getPaymasterData", [userOperation, entryPoint, chain, context]);
  }
  ...
  private async postJson(payload: unknown): Promise<JsonRequestResult> {
    ...
    const rateLimit = readRateLimitFromPayload(response.body) ?? readRateLimitFromHeaders(response.headers);
