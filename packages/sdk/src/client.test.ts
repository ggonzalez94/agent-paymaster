...
describe("ServoRpcClient", () => {
  ...
  it("sends pm_getPaymasterData with permit context", async () => {
    ...
    const result = await client.getPaymasterData(SAMPLE_USER_OPERATION, ENTRY_POINT, "taikoMainnet", {
      permit: {
        value: "1000000",
        deadline: "1900000000",
        signature:
  ...
  it("maps json-rpc error responses to JsonRpcRequestError", async () => {
    ...
    await expect(client.sendUserOperation(SAMPLE_USER_OPERATION, ENTRY_POINT)).rejects.toMatchObject<
  ...
  it("rejects invalid JSON-RPC payloads", async () => {
    ...
    await expect(client.supportedEntryPoints()).rejects.toMatchObject<Partial<AgentPaymasterSdkError>>({
